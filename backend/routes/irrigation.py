...
def get_coordinates(location: str):
    ...
    # Step 1 & 3: Try multiple query formats in order
    queries = [
        f"{location}, India",
        location
    ]
    ...
    # Step 4: Only then raise error
    if last_exception and not isinstance(last_exception, HTTPException):
         raise HTTPException(
            status_code=503,
            detail=f"Geocoding service unavailable: {str(last_exception)}"
        )
        
    raise HTTPException(
        status_code=404,
        detail=f"Location '{location}' not found. "
               f"Try a larger nearby city name."
    )
...
async def get_irrigation_schedule(req: IrrigationRequest):
    try:
        ...
        for i, date_str in enumerate(dates):
            ...
            wind_10m = wind_kmh / 3.6  # convert km/h to m/s
            wind = wind_10m * (4.87 / 2.3456)  # adjust from 10m to 2m height (FAO-56)
            # 2.3456 = ln(67.8 * 10 - 5.42)
            solar = daily["shortwave_radiation_sum"][i]  # already MJ/m²/day
        ...
        save_query("irrigation", req.dict(), response_data)
